  ...
  useEffect(() => {
    const trigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        // ── Canvas visibility ─────────────────────────────
        // clip1 / clipT: instant hard cuts at B1
        if (c1Ref.current) c1Ref.current.style.opacity = raw < B1 ? 1 : 0
        if (cTRef.current) cTRef.current.style.opacity = raw >= B1 && raw < BT ? 1 : 0

        // clip2 → clip3: scroll-driven perspective wipe at B2
        // TRANS_W: 1.8% of total scroll travel (~14vh out of 800vh)
        {
          const TRANS_W = 0.018
          const t = clamp01((raw - (B2 - TRANS_W * 0.5)) / TRANS_W)

          if (c2Ref.current && raw >= BT) {
            c2Ref.current.style.opacity = Math.max(0, 1 - t)
            c2Ref.current.style.transform = t > 0
              ? `perspective(900px) scale(${(1 - t * 0.07).toFixed(4)}) rotateY(${(-t * 7).toFixed(2)}deg) translateZ(${(-t * 55).toFixed(1)}px)`
              : ''
          } else if (c2Ref.current) {
            c2Ref.current.style.opacity = 0
            c2Ref.current.style.transform = ''
          }

          if (c3Ref.current) {
            c3Ref.current.style.opacity = t
            c3Ref.current.style.transform = t > 0 && t < 1
              ? `perspective(900px) scale(${(1 + (1 - t) * 0.04).toFixed(4)})`
              : ''
          }
        }
        ...
        // Beat 5 — frost payoff (loc3) — bottom-right, enters from right
        if (beat5Ref.current) {
  ...
  return (
    <section ref={containerRef} className="vs">
      <div ref={stageRef} className="vs__stage">
        {/* Brush cursor — white blob inverts colors on the dark stage */}
        <PaintBrush containerRef={stageRef} blendMode="overlay" color="#00d8ff" blobSize={300} maxWidth={90} />
